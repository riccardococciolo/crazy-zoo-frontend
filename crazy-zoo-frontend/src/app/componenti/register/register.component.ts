...
export class RegisterComponent {
  ...
  constructor(private authService: AuthService, private router: Router) {
    this.registerForm = this._formBuilder.group(
      {
        nome: [
          '',
          [Validators.required, Validators.pattern(/^[A-Za-zÀ-ÿ\s]{2,50}$/)],
        ],
        cognome: [
          '',
          [Validators.required, Validators.pattern(/^[A-Za-zÀ-ÿ\s]{2,50}$/)],
        ...
        username: [
          '',
          [Validators.required, Validators.pattern(/^[a-zA-Z0-9._-]{3,20}$/)],
        ],
        ...
        via: [
          ...
          [
            Validators.required,
            Validators.pattern(/^[A-Za-zÀ-ÿ0-9\s]{3,100}$/),
          ],
        ],
    ...
    ); // 👈 Aggiunto validatore personalizzato
  }
  ...
  register(): void {
    ...
    setTimeout(() => {
      if (this.authService.isRcReg === false) {
        this.showAlert = true;
        this.success = false;
