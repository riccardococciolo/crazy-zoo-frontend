...
export class OrdineComponent implements OnInit {
  ...
  exportCSV(): void {
    ...
    this.listOrdini.forEach((ordine: any) => {
      ...
      csvData.push(
        `${ordine.id};${ordine.utente.nome};${ordine.utente.cognome};${
          ordine.utente.email
        };${
          ordine.utente.indirizzo
            ? `${ordine.utente.indirizzo.via}, ${ordine.utente.indirizzo.civico}, ${ordine.utente.indirizzo.cap} ${ordine.utente.indirizzo.citta}`
            : 'N/A'
        };${ordine.totale}`
      );
      csvData.push(`Prodotto`);
      csvData.push(
        `IdProdotto;TitoloProdotto;PrezzoProdotto;QuantitaProdotto;MarcaProdotto;TipologiaProdotto`
      );
      ordine.prodotti.forEach((prodotto: any) => {
        csvData.push(
          `${prodotto.id};${prodotto.titolo};${prodotto.prezzo};${
            prodotto.quantita
          };${prodotto.marca ? prodotto.marca.nome : 'N/A'};${
            prodotto.tipologia ? prodotto.tipologia.nome : 'N/A'
          }`
        );
      });
      csvData.push('');
