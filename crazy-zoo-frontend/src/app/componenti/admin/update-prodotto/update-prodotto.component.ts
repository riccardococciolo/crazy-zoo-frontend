...
export class UpdateProdottoComponent {
  ...
  ngOnInit(): void {
    this.route.paramMap.subscribe((params: ParamMap) => {
      ...
      this.servP.getProdottoAll({ id: this.id }).subscribe((resp) => {
        this.response = resp;
        ...
        this.updateProdotto = new FormGroup({
          ...
          tipologia: new FormControl(
            this.data[0].tipologia.id,
            Validators.required
          ),
          animale: new FormControl(
            this.data[0].animale.id,
            Validators.required
          ),
          quantità: new FormControl(this.data[0].quantita, [
            Validators.required,
            Validators.min(1),
          ]),
          descrizione: new FormControl(this.data[0].descrizione, [
            Validators.required,
          ]),
